...
class TestWGAN_GP:
    ...
    def test_gradient_penalty_full(self, latent_dim, batch_size, device):
        ...
        with patch.object(wgan.discriminator, "forward") as mock_disc:
            ...
            gp_identical = wgan.compute_gradient_penalty(identical_samples, identical_samples)
            assert torch.isclose(gp_identical, torch.tensor(0.0, device=device), atol=1e-4)
        ...
        with patch.object(wgan.discriminator, "forward") as mock_disc:
            ...
            assert torch.isclose(gp_mocked, torch.tensor(1.0, device=device) * 10.0, atol=1e-4)
